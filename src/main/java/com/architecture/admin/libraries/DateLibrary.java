...
public class DateLibrary {
     ...
     * 기간 계산
     *
     * @param period : 기간 (1: 1개월, 12: 1년)
     * @return : startDate(현재 시간으로 부터 몇 개월 전), endDate(현재 시간)
     */
    public Map<String, String> getStartDateAndEndDateByPeriod(Integer period) {

        if (period == null || period < 1) {
            throw new CustomException(CustomError.DATE_PERIOD_EMPTY); // 기간이 비었습니다.
        }
        ...
        // 현재 시간으로 부터 period 개월 전의 시간
        Calendar cal = Calendar.getInstance();
        cal.setTime(Timestamp.valueOf(nowDate));
        cal.add(Calendar.MONTH, -period); // 몇 개월 전
        // startDate
        String startDate = formatDatetime.format(cal.getTime());
        startDate = startDate.substring(0, 10); // 시분초 제거
        startDate = startDate + " 00:00:00";     // 00:00:00 추가

        return Map.of("startDate", startDate, "endDate", nowDate);
    }
     ...
     *
     * @param date : 날짜
     * @return : true(날짜 형식), false(날짜 형식 아님)
     */
    public boolean isDate(String date) {
        SimpleDateFormat formatDatetime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            formatDatetime.parse(date);
            return true;
        } catch (Exception e) {
            return false;
        }
